...
// Code Block Component for API Documentation
...
function App() {
  ...
  const [newAgent, setNewAgent] = useState({ name: '', owner_email: '', connector_type: 'openai', description: '' })
  const [newPolicy, setNewPolicy] = useState({ name: '', policy_type: 'pii', rule_json: {}, description: '' })
  ...
  const createPolicy = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setNewPolicy({ name: '', policy_type: 'pii', rule_json: {}, description: '' })
  ...
  const evaluateGuardrails = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setEvaluationHistory([{ ...result, timestamp: new Date().toISOString(), input: evaluationInput }, ...evaluationHistory.slice(0, 19)])
